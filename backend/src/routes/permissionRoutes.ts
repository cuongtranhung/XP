...
router.get('/test', async (req: Request, res: Response) => {
  res.json({ success: true, message: 'Permission routes are loaded!' });
});
...
router.put('/roles/:roleId/permissions', authMiddleware, async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Add permissions
    if (add.length > 0) {
      const values = add.map((permId: string) => `('${roleId}', '${permId}')`).join(',');
      await client.query(`
